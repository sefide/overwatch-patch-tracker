...
class OverwatchPatchScraper {
    ...
    private fun parsePatchNotes(document: Document): List<PatchNoteDto> {
        ...
        val dateHeaders = document.select("h3")
    ...
    private fun parseHeroUpdatesFromSection(elements: List<Element>): List<HeroUpdateDto> {
        ...
        // h5 태그로 영웅 이름 찾기
        val heroSections = elements.filter { it.tagName() == "h5" }
        
        heroSections.forEach { heroHeader ->
            try {
                val heroName = cleanHeroName(heroHeader.text())
                
                // 개발자 코멘트 찾기
                val developerComment = findDeveloperComment(heroHeader)
                
                // 변경사항 파싱
                val changes = parseBalanceChanges(heroHeader)
                ...
                logger.error("Failed to parse hero section: ${heroHeader.text()}", e)
            }
        }
